...
numeric_cols <- setdiff(colnames(happy2020), c("Country.name", "Regional.indicator"))

# Calculate the mean, median, mode, and variance values for each numeric column
mean_values <- sapply(happy2020[, numeric_cols], mean)
median_values <- sapply(happy2020[, numeric_cols], median)
mode_values <- sapply(happy2020[, numeric_cols], mode)
variance_values <- sapply(happy2020[, numeric_cols], var)
